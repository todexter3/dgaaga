...
import pandas as pd
import data_provider.data_loader_kfold as data_loader
...
from utils.loss import WeightedMSELoss, CCC
import utils.plt_bw as plt_bw
...
class Exp_Multiple_Regression_Fold(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        if self.args.data_type == 'daily':
            if flag == 'train':
                train_dataset = data_loader.Dataset_regression_train_val(self.args)
                return train_dataset
            elif flag == 'test':
                test_dataset, test_loader = data_loader.Dataset_regression_test(self.args)
    ...
    def test(self, setting, test=0):
        ...
        for fold in range(self.args.num_fold):
            ...
            with torch.no_grad():
                ...
                f.write('\n')
                f.close()
                plt_bw.plt_epoch_train_val_trend_fold(self.args, f'{self.args.save_path}/_result_of_multiple_regression.txt') # 画train过程图
